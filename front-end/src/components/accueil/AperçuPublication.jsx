...
const AperçuPublication = ({ pub, liked, onLike, onClose, theme }) => {
  useEffect(() => {
    const detecterEchap = e => { if (e.key === 'Escape') onClose(); };
    ...
    >
      ...
      >
        ...
        <div className="md:w-[58%] bg-black/10 flex items-center justify-center overflow-hidden min-h-[260px] max-h-[45vh] md:max-h-[88vh]">
          <img src={pub.image_affichee} alt={pub.titre} className="w-full h-full object-contain p-2" />
        </div>
              ...
              {pub.est_vendue ? (
                ...
                <div className="flex items-center justify-between gap-4">
                  <div className="flex flex-col">
                    ...
                    <span style={{ color: theme.accentSage }} className="font-serif italic text-xl font-light">{pub.prix} €</span>
                  </div>
                  <button style={{ backgroundColor: theme.accentSage }} className="px-5 py-2.5 rounded-lg text-[#2d3a30] text-[9px] tracking-[0.25em] uppercase font-bold hover:bg-white hover:text-[#2d3a30] transition-all">
                    Acquérir
                  </button>
                </div>
              )}
          ...
          <div className="pt-5 border-t border-white/10 flex items-center justify-between">
            <div className="flex items-center gap-3">
              {pub.photographe_photo ? (
                <img src={pub.photographe_photo} alt={pub.photographe_nom} className="w-8 h-8 rounded-full object-cover border border-white/10" />
              ) : (
            ...
            <button onClick={e => onLike(pub.id, e)} className="flex items-center gap-2 group/like">
              <motion.span
                key={liked ? 'l' : 'u'}
                initial={{ scale: 0.8 }} animate={{ scale: 1 }}
                style={{ color: liked ? theme.accentSage : 'inherit' }}
                className={`text-base ${!liked && 'text-white/30 group-hover/like:text-white/50'}`}
              >{liked ? '♥' : '♡'}</motion.span>
